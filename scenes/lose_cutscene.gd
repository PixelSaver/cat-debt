...
func sfx_tween_in(ap: int, duration: float):
	...
	if ap == 1:
		...
		audio_player1.play()
		sfx_t.tween_property(audio_player1, "volume_db", 0, duration)
	elif ap == 2:
		audio_player2.volume_db = -50
		audio_player2.play()
		sfx_t.tween_property(audio_player2, "volume_db", 0, duration)
	
func sfx_tween_out(ap: int, duration: float):
	var sfx_t: Tween = create_tween()
	sfx_t.set_ease(Tween.EASE_OUT)
	sfx_t.set_trans(Tween.TRANS_QUINT).set_parallel(true)
	
	if ap == 1:
		audio_player1.volume_db = 0
